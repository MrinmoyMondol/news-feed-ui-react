...
import { Play,Pause , Mic, SquarePlay, Image as ImageIcon, Globe } from "lucide-react";
...
const SocialPost = ({
  ...
  return (
    <div className="max-w-[520px] mx-auto bg-white border border-gray-200 shadow-sm rounded-sm font-sans overflow-hidden">
      ...
      <div className="w-full">
        <img src={imageUrl} alt="Post Content" className="w-full h-auto object-cover block" />
      </div>
      ...
      <div className="flex items-center justify-between mt-[10px] bg-white px-[10px]">
        ...
        <button className="bg-[#e4e6eb] hover:bg-[#d8dadf] text-[#050505] px-[14px] py-[7px] rounded-[8px] text-[14px] font-bold transition-colors whitespace-nowrap">
          Create Post
        </button>
      </div>
      ...
      <div className="flex items-center mb-[10px]  justify-between py-[8px] bg-[#f8f9fb]">
        <div className="flex items-center px-[10px] -space-x-3.5">
          ...
          <span className="text-[18px] font-semibold px-[5px] text-[#1a2b3c] ml-1">{likes}</span>
        </div>

        <div className="flex items-center text-[#1a2b3c]">
          <div className="flex items-center gap-2.5 px-4">
            <SquarePlay size={22} strokeWidth={1.8} />
            <span className="text-[17px] px-[5px] font-semibold">{stats.plays}</span>
          </div>
          <div className="h-5 w-[1px] bg-gray-200"></div>
          <div className="flex items-center gap-2.5 px-4">
            <Mic size={22} strokeWidth={1.8} />
            <span className="text-[17px] px-[5px] font-semibold">{stats.mics}</span>
          </div>
          <div className="h-5 w-[1px] bg-gray-200"></div>
          <div className="flex items-center gap-2.5 px-4">
            <ImageIcon size={22} strokeWidth={1.8} />
            <span className="text-[17px] px-[5px] font-semibold">{stats.images}</span>
          </div>
        </div>
      </div>
